...
import type { AnalysisHistoryItem, DiagnosticItem, ImprovementSuggestion, StrengthItem, ComplianceBreakdown, ComplianceCategoryReport, ComplianceIssue } from '../types';
    ...
    <div className="bg-black/30 p-4 rounded-md border border-cyan-500/20 backdrop-blur-sm text-sm h-full">
        <div className="flex flex-col space-y-3">
            ...
            <div className="flex items-center text-gray-400 space-x-2">
                ...
                <span>分析時間: <span className="font-semibold text-gray-300">{date}</span></span>
            </div>
             {modelUsed && (
                 <div className="flex items-center text-gray-400 space-x-2">
...
const StrategyCard: React.FC<StrategyCardProps> = ({ item, videoTheme }) => {
  ...
  const handleCopyOriginal = (e: React.MouseEvent) => {
    handleCopy(e, item.actionableItem);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };
  
  const handleCopySuggestion = (e: React.MouseEvent, text: string, index: number) => {
    handleCopy(e, text);
  ...
  const handleRewrite = async (e: React.MouseEvent) => {
    ...
    try {
      const newSuggestions = await generateCopySuggestions(item.actionableItem, item.type, videoTheme);
      ...
      setRewriteError('AI 建議生成失敗，請稍後再試。');
    } finally {
  ...
  return (
    <div className="bg-black/30 p-4 rounded-md border border-cyan-500/20 backdrop-blur-sm flex flex-col">
        ...
        <div className="flex-grow">
          ...
          <div className="bg-gray-900/50 p-3 rounded-sm text-sm text-gray-300 whitespace-pre-wrap relative group">
            ...
            <div className="absolute top-2 right-2 flex space-x-1 opacity-60 group-hover:opacity-100 transition-opacity">
               {(item.type === 'Hook' || item.type === 'CTA') && (
                 <button onClick={handleRewrite} disabled={isRewriting} className="p-1.5 bg-gray-700 rounded-sm hover:bg-gray-600 disabled:opacity-50 disabled:cursor-wait" title="AI 改寫建議">
                    {isRewriting ? <SpinnerIcon className="w-4 h-4" /> : <ZapIcon className="w-4 h-4 text-yellow-400"/>}
                 </button>
               )}
               <button onClick={handleCopyOriginal} className="p-1.5 bg-gray-700 rounded-sm hover:bg-gray-600">
      ...
      {(isRewriting || rewriteError || suggestions.length > 0) && (
        <div className="mt-4 pt-4 border-t border-cyan-500/10">
          {isRewriting && !rewriteError && (
             <div className="flex items-center justify-center p-4">
                 <SpinnerIcon className="w-5 h-5 text-cyan-400 mr-2" />
                 <span className="text-sm text-cyan-400">AI 正在生成建議...</span>
             </div>
          )}
          {rewriteError && (
            <p className="text-sm text-red-400 text-center">{rewriteError}</p>
          )}
          {suggestions.length > 0 && (
            <div className="space-y-2">
               <h5 className="text-sm font-semibold text-gray-300 mb-2">AI 改寫建議：</h5>
               {suggestions.map((suggestion, index) => (
                   <div key={index} className="bg-gray-900/70 p-2 rounded-sm text-sm text-gray-300 relative group/suggestion">
                       <code>{suggestion}</code>
                       <button onClick={(e) => handleCopySuggestion(e, suggestion, index)} className="absolute top-1/2 -translate-y-1/2 right-2 p-1.5 bg-gray-700 rounded-sm hover:bg-gray-600 opacity-0 group-hover/suggestion:opacity-100 transition-opacity" title="複製建議">
                         {copiedSuggestion === index ? <CheckCircleIcon className="w-4 h-4 text-green-400"/> : <ClipboardIcon className="w-4 h-4"/>}
                       </button>
                   </div>
               ))}
            </div>
...
const SafeAreaOverlay: React.FC = () => (
    <div className="absolute inset-0 pointer-events-none text-xs uppercase tracking-widest">
        ...
        {/* Corner brackets for the whole 9:16 view */}
...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ analysisItem, currentUser, onTogglePublic }) => {
  ...
  const handleExport = async () => {
    ...
    try {
        const canvas = await html2canvas(reportContentRef.current, {
            useCORS: true,
            backgroundColor: '#030712', // gray-950, matches the background
        ...
        const canvasPageHeight = pdfWidth / ratio;
        let position = 0;
        let heightLeft = imgHeight;

        pdf.addImage(imgData, 'PNG', 0, position, pdfWidth, canvasPageHeight);
        ...
        pdf.save(`${analysisItem.videoName}_report.pdf`);
    } catch (error) {
  ...
  const prioritySuggestions = useMemo(() => {
    ...
    const priorityScore = (item: DiagnosticItem) => {
        const impactScores = { high: 3, medium: 2, low: 1 };
        const fixScores = { 'Text/Graphics': 5, 'Color/Lighting': 3, 'Audio': 3, 'Pacing': 2, 'Re-edit': 1 };
        return (impactScores[item.impact] || 0) * (fixScores[item.fixType as keyof typeof fixScores] || 0);
    };
  ...
  return (
    <div>
        ...
        <div ref={reportContentRef} className="flex flex-col space-y-8 pb-12">
              ...
              <div className="flex flex-col space-y-4 w-full max-w-sm mx-auto">
                <div className="flex flex-col space-y-2">
                    <div className="relative overflow-hidden rounded-md border border-cyan-500/20 shadow-[0_0_15px_rgba(56,189,248,0.2)] bg-black aspect-[9/16] w-full">
                      <video ref={videoRef} src={videoUrl} controls className="w-full h-full object-cover"></video>
                    ...
                    <div className="flex items-center justify-between bg-black/30 border border-cyan-500/20 px-3 py-1.5 rounded-md w-full backdrop-blur-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        {[0.75, 1, 1.25].map(rate => (
                          <button key={rate} onClick={() => setPlaybackRate(rate)} className={`px-2 py-0.5 text-xs rounded-sm ${playbackRate === rate ? 'bg-cyan-500 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                            {rate}x
                          </button>
                        ))}
                      </div>
          ...
          {/* --- SECTION 3: COMPLIANCE DEEP DIVE --- */}
          <section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div>
                  ...
                  <div className="flex flex-col space-y-4">
                      {!preliminaryResult ? (
                          Array.from({ length: 4 }).map((_, i) => <SkeletonCard key={i} className="min-h-[10rem]" />)
                      ) : (
                          <>
                              ...
                              <InfoCard title="場景標籤" icon={<TagIcon className="w-6 h-6 text-cyan-400"/>}>
                                  <div className="flex flex-wrap gap-2">
                                      {preliminaryResult.sceneTags?.length > 0 ? (
                                          preliminaryResult.sceneTags.map((tag, index) => (
                                              <span key={index} className="bg-cyan-900/50 text-cyan-300 text-sm font-medium px-3 py-1 rounded-sm">{tag}</span>
                                          ))
                                      ) : <p className="text-gray-500">未偵測到場景標籤。</p>}
                                  </div>
                              </InfoCard>
                              <InfoCard title="潛在風險詞" icon={<ShieldAlertIcon className="w-6 h-6 text-red-400"/>}>
                                  <div className="flex flex-wrap gap-2">
                                       {preliminaryResult.riskWords?.length > 0 ? (
                                          preliminaryResult.riskWords.map((word, index) => (
                                              <span key={index} className="bg-red-900/50 text-red-300 text-sm font-medium px-3 py-1 rounded-sm">{word}</span>
                                          ))
                                      ) : <p className="text-gray-500">未偵測到潛在風險詞。</p>}
                                  </div>
                              </InfoCard>
                ...
                <div>
                  ...
                  {fullResult?.diagnostics
                      ? <>
                          <p className="text-sm text-gray-500 mb-4">點擊時間戳記可跳轉至影片對應時間點。</p>
          ...
          {/* --- SECTION 5: STRATEGIC TOOLBOX --- */}
          <section>
            <div>
                <h3 className="text-lg sm:text-xl font-bold mb-4 text-cyan-300 tracking-wider">策略改良建議</h3>
                {fullResult?.improvementPackage && preliminaryResult
                    ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {fullResult.improvementPackage.map(item => (
                            <StrategyCard 
                                key={item.type}
                                item={item}
                                videoTheme={preliminaryResult.coreTheme}
                            />
                        ))}
                      </div>
                    : <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><SkeletonCard className="min-h-[12rem]" /><SkeletonCard className="min-h-[12rem]" /></div>
                }
            </div>
          </section>
