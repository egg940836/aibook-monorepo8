...
const getStatusBadge = (status: AnalysisHistoryItem['status']) => {
    switch (status) {
        ...
        case 'error':
             return <span className="inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium bg-red-500/20 text-red-300">錯誤</span>;
        default:
        ...
        <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-cyan-500/20">
                ...
                {analyses.map((item) => (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{new Date(item.date).toLocaleString('zh-TW')}</td>
